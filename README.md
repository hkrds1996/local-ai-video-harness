...
The project explores a narrow engineering question: how much of a video-generation workflow can run locally, without per-generation video API charges and without manual browser interaction?

The repository provides a small English demonstration manifest and a generic ComfyUI API runner. It intentionally separates public project intent from private model paths and machine configuration.

## Project status

This is an early, working foundation rather than a complete video editor.

Implemented:

- static validation of project manifests;
- execution-plan summaries that require no GPU;
- loading of ComfyUI API-format workflows;
...
```json
{
  ...
  "workflow_api": "C:/path/to/api-format-workflow.json",
  "output_dir": "outputs",
  "prompt_node": "12",
  "prompt_input": "text",
  "seed_node": "15",
...
2. injects the shot prompt and seed;
3. submits the workflow to `/prompt`;
...
The output directory contains a `state.json` file with one record per successful shot. It stores the backend prompt identifier and downloaded file paths.
